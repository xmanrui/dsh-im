...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === WHATSAPP_ENDPOINTS.setIsolateConversationWorkspace) {
    return validIsolateWorkspacePayload(payload)
      ? null : '请提交有效的工作区隔离设置。';
  }
  if (endpoint === WHATSAPP_ENDPOINTS.setIsolateConversationGuidance) {
...
export function createWhatsappRpcHandler(controller, { encodeQr = qrDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === WHATSAPP_ENDPOINTS.setIsolateConversationWorkspace) {
      ...
      } else if (endpoint === WHATSAPP_ENDPOINTS.setIsolateConversationGuidance) {
