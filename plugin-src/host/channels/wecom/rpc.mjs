...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === WECOM_ENDPOINTS.setIsolateConversationWorkspace) {
    return validIsolateWorkspacePayload(payload)
      ? null : '请提交有效的工作区隔离设置。';
  }
  if (endpoint === WECOM_ENDPOINTS.setIsolateConversationGuidance) {
...
export function createWecomRpcHandler(controller, { encodeQr = qrDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.setIsolateConversationWorkspace) {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.setIsolateConversationGuidance) {
