...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === WECOM_ENDPOINTS.setDefaultModel) {
    return validDefaultModelPayload(payload)
      ? null : '请提交有效的默认模型设置。';
  }
  if (endpoint === WECOM_ENDPOINTS.modelCatalog) {
...
export function createWecomRpcHandler(controller, { encodeQr = qrDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.setDefaultModel) {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.modelCatalog) {
