...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === WECOM_ENDPOINTS.setContextEnhancement) {
    ...
    if (endpoint === WECOM_ENDPOINTS.setConversationDirectory) {
    return validConversationDirectoryPayload(payload)
      ? null : '请输入有效的会话目录设置。';
  }
  if (endpoint === WECOM_ENDPOINTS.setConversationDirectoryDefault) {
...
export function createWecomRpcHandler(controller, { encodeQr = qrDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.setConversationDirectory) {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.setConversationDirectoryDefault) {
