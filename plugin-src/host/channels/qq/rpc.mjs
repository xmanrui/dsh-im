...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === QQ_ENDPOINTS.setDefaultModel) {
    return validDefaultModelPayload(payload)
      ? null : '请提交有效的默认模型设置。';
  }
  if (endpoint === QQ_ENDPOINTS.modelCatalog) {
...
export function createQqRpcHandler(controller, { encodeQr = qrDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === QQ_ENDPOINTS.setDefaultModel) {
      ...
      } else if (endpoint === QQ_ENDPOINTS.modelCatalog) {
