...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === SLACK_ENDPOINTS.setContextEnhancement) {
    ...
    if (endpoint === SLACK_ENDPOINTS.setConversationDirectory) {
    return validConversationDirectoryPayload(payload)
      ? null : '请输入有效的会话目录设置。';
  }
  if (endpoint === SLACK_ENDPOINTS.setConversationDirectoryDefault) {
...
export function createSlackRpcHandler(controller) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      else if (endpoint === SLACK_ENDPOINTS.setConversationDirectory) {
      ...
      else if (endpoint === SLACK_ENDPOINTS.setConversationDirectoryDefault) {
