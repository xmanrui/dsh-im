...
const EN = Object.freeze({
  ...
  '语音交互': 'Voice',
  '查看语音交互说明': 'View voice help',
  '语音交互开关': 'Voice on/off',
  '开启(语音提问与语音回复)': 'On (voice questions and voice replies)',
  '收到语音消息时转写为文字并按文字流水线处理,答案同时以语音回复;转写失败时降级为仅支持文字/图片/文件的提示。凭据值在插件凭据库里管理,这里只引用环境变量名。': 'Voice messages are transcribed to text and processed by the normal text pipeline; the answer is also sent as a voice reply. If transcription fails, the bot falls back to the text/image/file-only notice. The credential value lives in the plugin credential store; only the environment variable name is referenced here.',
  '凭据引用(环境变量名)': 'Credential reference (environment variable name)',
  '转写模型(ASR)': 'Transcription model (ASR)',
  '合成模型(TTS)': 'Synthesis model (TTS)',
  '回复音色': 'Reply voice',
  '需要 DashScope 密钥(凭据库中以"凭据引用"为名的环境变量)与可用 ffmpeg;音色支持 Cherry/Serena/Bella/Chelsie/Momo/Katerina 等 qwen3-tts-flash 官方音色。': 'Requires a DashScope API key (the credential-store environment variable named by the credential reference above) and a working ffmpeg. Supported voices include the official qwen3-tts-flash voices Cherry/Serena/Bella/Chelsie/Momo/Katerina.',
  '保存语音设置': 'Save voice settings',
  '默认关闭;开启后机器人可听懂语音消息并以语音回复。': 'Off by default. When enabled, the bot understands voice messages and replies by voice.',
  '语音设置保存失败，请重试。': 'Could not save the voice settings. Try again.',
