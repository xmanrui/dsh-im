...
| `/workspace <workspace index or absolute path>`, `/ws <workspace index or absolute path>` | Switch the Harness workspace by `/workspacelist` index or absolute path. By default this is bot-wide; enable **Isolate workspace per chat** on the settings card to affect only the current chat. |
| `/guidance [text or --clear]`, `/prompt [...]` | Show or set enhancement guidance. By default this writes the bot-wide group or direct-chat guidance; enable **Isolate guidance per chat** on the settings card to affect only the current chat. `--clear` follows the default; `--none` sends this chat with no guidance. |
...
- **Image understanding**: all nine built-in channels can send JPEG, PNG, WebP, and GIF files sent as images to Harness, with an optional text description. Each image is limited to 5 MB, and all images in one message are limited to 20 MB in total. In DingTalk group chats, picking a file sends it immediately: send the file first, then @the bot with what to do; or reply to that file and @the bot. The held attachment lasts about 10 minutes and is limited to the same sender.
- **Switch workspaces from a bot card**: every bot card on the settings page shows that bot's default Harness workspace. Enter an existing absolute directory path directly or open the directory picker. The card also offers **Isolate workspace per chat**: when on, `/workspace` in a group or direct chat overrides only that chat; when off, every chat of the bot shares one workspace. Switching clears only the affected chat mappings; it never deletes, empties, or archives old Sessions. Replies already in progress may finish.
- **Isolate guidance per chat**: the **Isolate guidance per chat** switch on a bot card is off by default. When on, `/guidance` overrides only the current chat; when off, the bot still shares one group guidance and one direct-chat guidance. The settings-card context-enhancement editor always edits the defaults. The group or direct enable switch must still be on for guidance to be injected.
