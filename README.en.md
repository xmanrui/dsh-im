...
| WeCom app | Create a self-built app in the WeCom admin console and enter Corp ID, AgentId, Secret, Token and EncodingAESKey (proxy address optional) | HTTP callback; direct messages stream replies and fall back to chunked text where the WeChat client cannot, with image input and result files returned; members reach it from WeChat itself after following the enterprise's WeChat plugin |
...
- Uses logos for WeChat, Feishu, DingTalk, WeCom, WeCom app, QQ, Slack, Telegram, Discord, WhatsApp, iMessage, Matrix, and AI Office navigation without enable/disable switches.
