...
**分步直推（Step Push）**：
开启后回合内的工具调用与助手中间说明以独立消息逐条推送到会话路由、最终答案以富文本消息推送的呈现模式。它是详细级别过程呈现的开关，默认关闭保持原流式模式。
...
**思考中状态（Thinking Status）**：
分步直推回合内的静默期心跳——距上一条已推送消息静默超过阈值时出现、原地刷新运行时长，并在真实事件或回合结束时自动撤回的单条状态消息。
_Avoid_: 心跳刷屏、保活消息、卡死提示
...
**基工作区（Base Workspace）**：
一个对话用于派生其会话目录的工作区；它由 bot 默认工作区或该对话的显式工作区覆盖决定，本身可以是任何绝对路径。
_Avoid_: 父目录、默认目录、根工作区

**会话目录（Conversation Directory）**：
由会话标识确定性推导、位于基工作区之下、并在新建 Harness Session 之前成为该对话工作目录的目录；同一对话按策略复用或逐会话新建，插件不删除已建目录。
_Avoid_: 临时目录、工作区副本、沙箱目录

**会话目录隔离（Conversation Directory Isolation）**：
按 bot 开启的能力：新建会话前先建会话目录并切换工作目录，使不同对话的文件与命令互不覆盖；未开启时插件行为与开启前一致。
_Avoid_: 目录沙箱、会话隔离模式、默认行为
