...
function delivererFixture({
  ...
  proactiveError = false,             // true 则第三级主动推送也失败（全链失败场景）
} = {}) {
  const listeners = [];
  const sent = [];
  const proactive = [];
  ...
  const api = {
    sendRobotText: async ({ target, text }) => {
      if (proactiveError) throw new Error('proactive push rejected');
...
test('a fully failed delivery chain still releases ownership at turn end', async () => {
  // 回归（上游评审点 4）：卡片 → webhook → 主动推送三级全部失败时，
  // turn 控制权也必须在终态确认后无条件释放，不得残留在进程级 registry。
  const fx = delivererFixture({
    sendText: async () => { throw new Error('webhook rejected'); },
    proactiveError: true,
  });
  const deferred = deferredFixture();
  await fx.deliverer.register({ key: 'p2p:staff-approved', deferred, route: P2P_ROUTE });
  assert.equal(deferred.released, 1, 'ownership must release once the turn reached a terminal state');
  assert.equal(fx.remembered.length, 0, 'nothing was delivered, nothing to remember');
  // 条目已消费：重复事件不得重复投递或重复释放。
  fx.listeners[0].onSessionEvent({
    sessionId: 'session-defer',
    event: { type: 'turn/end', seq: 4, data: { turn: 1, reason: { kind: 'completed' } } },
  });
...
test('dispose releases ownership of pending entries', async () => {
  // 停机兜底：交付器不持久化，但 turn 控制权注册在进程级 registry，
  // 插件停止时必须显式释放，否则重启后成为孤儿 ownership。
  const fx = delivererFixture({ history: historyFixture({ ended: false }) });
  const deferred = deferredFixture();
  await fx.deliverer.register({ key: 'p2p:staff-approved', deferred, route: P2P_ROUTE });
  assert.equal(deferred.released, 0, 'turn still running: ownership must stay for /stop');
  fx.deliverer.dispose();
  assert.equal(deferred.released, 1);
  assert.equal(fx.deliverer.pendingFor('p2p:staff-approved', 'session-defer'), false);
});
