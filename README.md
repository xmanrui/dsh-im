...
| `/workspace <工作区序号或绝对路径>`、`/ws <工作区序号或绝对路径>` | 按 `/workspacelist` 序号或绝对路径切换 Harness 工作区。默认作用于整台机器人；在设置页开启「按聊天隔离工作区」后，只影响当前聊天。 |
| `/guidance [提示词 或 --clear]`、`/prompt [...]` | 查看或设置增强提示词。默认改整台机器人的群聊或私聊提示词；在设置页开启「按聊天隔离提示词」后，只影响当前聊天。`--clear` 跟随默认，`--none` 当前聊天不附加提示词。 |
...
- **图片识别**：九个内置渠道都可以把 JPEG、PNG、WebP，以及以图片文件方式发送的 GIF 交给 Harness；图片可以附带文字说明。单张图片上限为 5 MB，单条消息中的图片总大小上限为 20 MB。钉钉群聊点选文件会立即发出：先发文件，再 @ 机器人说明要做什么；也可以回复该文件后 @。暂存约 10 分钟，且仅限同一发送者。
- **在机器人卡片切换工作区**：设置页中的每张机器人卡片都会显示该机器人的默认 Harness 工作区。可以直接填写已有目录的绝对路径，也可以打开目录选择器。卡片上还可开启「按聊天隔离工作区」：开启后，群聊或私聊里的 `/workspace` 只覆盖当前聊天；关闭时仍整台机器人共用一个工作区。切换只清除受影响聊天的映射，不会删除、清空或归档旧 Session；已经开始的回复可以继续完成。
- **按聊天隔离提示词**：机器人卡片上的「按聊天隔离提示词」默认关闭。开启后，`/guidance` 只覆盖当前聊天；关闭时仍整台机器人共用群聊、私聊各一份提示词。设置页的上下文增强编辑器始终改默认提示词。群聊或私聊的启用开关仍须打开，增强提示词才会注入。
