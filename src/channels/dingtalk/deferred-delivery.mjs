...
export function createDeferredDeliverer({
  ...
  async function scanEntry(entry) {
    ...
    const task = (async () => {
      ...
      // turn 已确认终态：turn 控制权（/stop 语义）到此结束。释放不依赖
      // 投递结果——卡片、webhook 与主动推送全部失败时 ownership 也不得残留。
      ...
      try {
        ...
        // 三级投递全链失败（范围裁定：不重试）。结果保留在 session history，
        // /session 切回后可用 /history 查看；ownership 已释放，无残留。
        logger.warn?.(
          '[dsh-dingtalk] deferred delivery failed after turn finished:',
          error?.message ?? error,
        );
      }
    })().catch((error) => {
  ...
  // 停机兜底：释放全部在途条目的 ownership。交付器不持久化（范围裁定），
  // 条目随实例消失，但 turn 控制权注册在 harness-client 的进程级 registry，
  // 必须显式释放，否则插件停止后成为孤儿（新实例无法再接管）。
  function dispose() {
    for (const entry of [...entries.values()]) {
      entry.deferred.releaseOwnership?.();
    }
  ...
  watcherSignal.addEventListener('abort', dispose, { once: true });

  return { register, pendingFor, dispose };
}
