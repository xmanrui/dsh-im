...
// src/channels/shared/inbound-retention.mjs
var INBOUND_FILE_RETENTIONS = Object.freeze({
  turn: "turn",
  forever: "forever"
});
function normalizeInboundRetention(value) {
  if (value === void 0 || value === null || value === "") return "turn";
...
// plugin-src/client/inbound-attachment.js
var React8 = __toESM(require("react"), 1);
var RETENTION_OPTIONS = Object.freeze([
  Object.freeze({
    value: "turn",
...
function InboundAttachmentEditor({
  retention,
  busy = false,
  onSave,
  onClear
}) {
  const [confirmingClear, setConfirmingClear] = React8.useState(false);
  const current = retention === "forever" ? "forever" : "turn";
  React8.useEffect(() => {
    if (!confirmingClear) return void 0;
    const timer = setTimeout(() => setConfirmingClear(false), 8e3);
    return () => clearTimeout(timer);
  }, [confirmingClear]);
  return h2(
    "div",
    { className: "dim-preset dim-inboundRetention" },
    h2(
      "div",
      { className: "dim-presetHeader dim-inboundRetentionHeader" },
      h2("span", { className: "dim-presetTitle" }, "\u5165\u7AD9\u9644\u4EF6\u4FDD\u7559\u7B56\u7565"),
      onClear ? h2("button", {
        type: "button",
    ...
    }, RETENTION_OPTIONS.map((option) => h2("option", {
      key: option.value,
      value: option.value
    }, option.label)))
  );
    ...
    model.phase === "loading" ? h2(LoadingView) : model.phase === "error" ? h2(
      ...
      model.bots.length > 0 ? h2(AccountList, {
        ...
        onInboundRetentionSave: (account, retention) => saveBotSetting(
          account,
          "inbound-retention",
          DINGTALK_ENDPOINTS.setInboundRetention,
          { retention }
        ),
        onClearInboundAttachments: (account) => saveBotSetting(
          account,
          "inbound-attachments",
          DINGTALK_ENDPOINTS.clearInboundAttachments,
          {}
        ),
...
function createTokenChannelSettings(definition) {
  ...
  function AccountCard5({ account, busy, testNotice, removing, onReconnect, onWorkspaceSave, onAgentPresetSave, onContextEnhancementSave, onInboundRetentionSave, onClearInboundAttachments, onAccountSettingsSave, onRequestRemove, onConfirmRemove, onCancelRemove }) {
    const state = busy === "reconnect" ? "connecting" : account.state;
    const tone = account.connected ? "success" : state === "error" ? "error" : "warning";
    const stateLabel2 = account.connected ? "\u8FD0\u884C\u6B63\u5E38" : state === "connecting" ? "\u6B63\u5728\u8FDE\u63A5" : "\u8FDE\u63A5\u672A\u5C31\u7EEA";
    const summary2 = account.error?.message ?? (account.connected ? null : account.health.summary);
    ...
    model.phase === "loading" ? h2(LoadingView2) : model.phase === "error" ? h2(PageError, {
      ...
      model.bots.length > 0 ? h2(BotList, {
        ...
        onInboundRetentionSave: (connection, retention) => saveBotSetting(
          connection,
          "inbound-retention",
          FEISHU_ENDPOINTS.setInboundRetention,
          { retention }
        ),
        onClearInboundAttachments: (connection) => saveBotSetting(
          connection,
          "inbound-attachments",
          FEISHU_ENDPOINTS.clearInboundAttachments,
          {}
        ),
    ...
    model.phase === "loading" ? h2(LoadingView5) : model.phase === "error" ? h2(
      ...
      model.bots.length > 0 ? h2(AccountList2, {
        ...
        onInboundRetentionSave: (account, retention) => saveBotSetting(
          account,
          "inbound-retention",
          WEIXIN_ENDPOINTS.setInboundRetention,
          { retention }
        ),
        onClearInboundAttachments: (account) => saveBotSetting(
          account,
          "inbound-attachments",
          WEIXIN_ENDPOINTS.clearInboundAttachments,
          {}
        ),
        onRequestRemove: (account) => setRemoveTarget(account.botId),
        onConfirmRemove: (account) => void remove(account),
